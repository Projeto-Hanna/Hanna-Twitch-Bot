...
const tokenData = JSON.parse(
  await fs.readFile('./twitch-auth-tokens.json', 'utf-8'),
);
await authProvider.addUserForToken(tokenData, ['chat']);

authProvider.onRefresh(async (_userId, newTokenData) =>
  fs.writeFile(
    `./twitch-auth-tokens.json`,
...
bot.onResub(({ broadcasterName, userName, months }) => {
  bot.say(
    broadcasterName,
    `Obrigada @${userName} por ${months} mês/meses de sub!!`,
  );
});
...
bot.onSubGift(({ broadcasterName, gifterName, userName }) => {
  bot.say(
    broadcasterName,
    `Obrigada @${gifterName} por dar um sub para @${userName}!!`,
  );
});
